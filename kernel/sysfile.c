...
uint64 
sys_symlink(void){
  char tf[MAXPATH],path[MAXPATH];
  struct inode *ip;

  if (argstr(0, tf, MAXPATH) < 0 ) {
    return -1;
  }
  ...
  ilock(ip);
  // write the target file path name into the end of inode
  if (writei(ip, 0, (uint64)tf, ip->size, MAXPATH) != MAXPATH) {
    panic("symlink");
  }
  iunlockput(ip);
  end_op();
